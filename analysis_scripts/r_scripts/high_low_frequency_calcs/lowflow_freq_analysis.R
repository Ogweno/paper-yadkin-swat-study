...
#my_model_p_list=c(0.99,0.95,0.9,0.8,0.7,0.6,0.5,0.4,0.2,0.1,0.08,0.06,0.04,0.03,0.02,0.01)
miroc_baseline_model_lowfow_calcs=model_freq_calcs_all_rchs(miroc_baseline_obs_lowflow_calcs,kn_table,my_model_p_list,0.4,"lowflow")
...
csiro_baseline_obs_lowflow_calcs_nozeros=csiro_baseline_obs_lowflow_calcs %>% filter(obs_min_flow_cms_adj>0)
miroc_baseline_obs_lowflow_calcs_nozeros=miroc_baseline_obs_lowflow_calcs %>% filter(obs_min_flow_cms_adj>0)
hadley_baseline_obs_lowflow_calcs_nozeros=hadley_baseline_obs_lowflow_calcs %>% filter(obs_min_flow_cms_adj>0)
...
csiro_baseline_obs_lowflow_calcs_zero_count=csiro_baseline_obs_lowflow_calcs %>%
  group_by(RCH) %>% summarise(cum_num_zero_entries=sum(obs_min_flow_cms_adj==0)) %>%
  mutate(num_zero_entries_per_yr=cum_num_zero_entries/csiro_baseline_num_yrs,
         dataset="csiro_backcast")

miroc_baseline_num_yrs=length(unique(miroc_baseline_rch_data$YR))
miroc_baseline_obs_lowflow_calcs_zero_count=miroc_baseline_obs_lowflow_calcs %>%
  group_by(RCH) %>% summarise(cum_num_zero_entries=sum(obs_min_flow_cms_adj==0)) %>%
  mutate(num_zero_entries_per_yr=cum_num_zero_entries/miroc_baseline_num_yrs,
         dataset="miroc_backcast")

hadley_baseline_num_yrs=length(unique(hadley_baseline_rch_data$YR))
hadley_baseline_obs_lowflow_calcs_zero_count=hadley_baseline_obs_lowflow_calcs %>%
  group_by(RCH) %>% summarise(cum_num_zero_entries=sum(obs_min_flow_cms_adj==0)) %>%
  mutate(num_zero_entries_per_yr=cum_num_zero_entries/hadley_baseline_num_yrs,
         dataset="hadley_backcast")
...
miroc8_5_obs_lowflow_calcs_zero_count=miroc8_5_obs_lowflow_calcs %>% 
  ...
  mutate(num_zero_entries_per_yr=cum_num_zero_entries/miroc_baseline_num_yrs,
         dataset="miroc8_5_projection")

# bind all together
all_models_zero_counts=bind_rows(csiro_baseline_obs_lowflow_calcs_zero_count,
                                 miroc_baseline_obs_lowflow_calcs_zero_count,
                                 hadley_baseline_obs_lowflow_calcs_zero_count,
...
# ---- 5.2. export results for kelly ----

all_model_zero_counts_kelly=all_models_zero_counts %>%
  filter(RCH==8 | RCH==10 | RCH==18 | RCH==28)

# export to results
#setwd("/Users/ssaia/Documents/sociohydro_project/analysis/results/r_outputs")
#write_csv(all_model_zero_counts_kelly,"zero_flow_counts_for_kelly_with_backcast.csv")
