...
remove_outliers=function(obs_flow_input_df, kn_table, flow_option) {
  ...
  if (flow_option == "high") {
    ...
    input_flow_log <- obs_flow_input_df$max_annual_logq_logcms
    ...
    if (input_cskew >= -0.4 & input_cskew <= 0.4) {
      ...
      if (input_cskew < -0.4) {
        ...
        #output new df
        obs_output_df <- obs_flow_input_df %>% 
          filter(max_annual_logq_logcms > input_lowflow_limit)
        ...
        #output new df
        obs_output_df <- obs_flow_input_df %>% 
          filter(max_annual_logq_logcms < input_highflow_limit)
    ...
    input_flow_log <- obs_flow_input_df$min_annual_logq_logcms
    input_mean <- mean(input_flow_log)
    ...
    if (input_cskew >= -0.4 & input_cskew <= 0.4) {
      ...
      if (input_cskew < -0.4) {
        ...
        #output new df
        obs_output_df <- obs_flow_input_df %>% 
          filter(min_annual_logq_logcms > input_lowflow_limit)
        ...
        #output new df
        obs_output_df <- obs_flow_input_df %>%
          filter(min_annual_logq_logcms < input_highflow_limit)
