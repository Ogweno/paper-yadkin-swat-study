...
csiro_baseline_rch_raw_data=read_table2("CSIRO-output.rch",col_names=FALSE,skip=9) # backcast .rch file from SWAT
miroc_baseline_rch_raw_data=read_table2("MIROC-output.rch",col_names=FALSE,skip=9) # backcast .rch file from SWAT
hadley_baseline_rch_raw_data=read_table2("Hadley-output.rch",col_names=FALSE,skip=9) # backcast .rch file from SWAT
...
# reach file (.rch) 
# baseline data
csiro_baseline_rch_data=reformat_rch_file(csiro_baseline_rch_raw_data)
miroc_baseline_rch_data=reformat_rch_file(miroc_baseline_rch_raw_data)
hadley_baseline_rch_data=reformat_rch_file(hadley_baseline_rch_raw_data)
...
# baselines (=backcasts)
csiro_baseline_obs_calcs=obs_freq_calcs_all_rchs(csiro_baseline_rch_data,1,"flood")
my_model_p_list=c(0.99,0.95,0.9,0.8,0.7,0.6,0.5,0.4,0.2,0.1,0.08,0.06,0.04,0.03,0.02,0.01)
csiro_baseline_model_calcs=model_freq_calcs_all_rchs(csiro_baseline_obs_calcs,kn_table,my_model_p_list,0.4,"flood")

miroc_baseline_obs_calcs=obs_freq_calcs_all_rchs(miroc_baseline_rch_data,1,"flood")
#my_model_p_list=c(0.99,0.95,0.9,0.8,0.7,0.6,0.5,0.4,0.2,0.1,0.08,0.06,0.04,0.03,0.02,0.01)
miroc_baseline_model_calcs=model_freq_calcs_all_rchs(miroc_baseline_obs_calcs,kn_table,my_model_p_list,0.4,"flood")

hadley_baseline_obs_calcs=obs_freq_calcs_all_rchs(hadley_baseline_rch_data,1,"flood")
#my_model_p_list=c(0.99,0.95,0.9,0.8,0.7,0.6,0.5,0.4,0.2,0.1,0.08,0.06,0.04,0.03,0.02,0.01)
hadley_baseline_model_calcs=model_freq_calcs_all_rchs(hadley_baseline_obs_calcs,kn_table,my_model_p_list,0.4,"flood")
...
ggplot() +
  geom_point(aes(x=obs_return_period_yr,y=obs_max_flow_cms_adj),csiro_baseline_obs_calcs,size=1,color="orange") +
  geom_point(aes(x=obs_return_period_yr,y=obs_max_flow_cms_adj),miroc_baseline_obs_calcs,size=1,color="green") +
  geom_point(aes(x=obs_return_period_yr,y=obs_max_flow_cms_adj),hadley_baseline_obs_calcs,size=1,color="blue") +
  geom_line(aes(x=model_return_period_yr,y=model_flow_cms),csiro_baseline_model_calcs,color="orange") +
  geom_line(aes(x=model_return_period_yr,y=model_flow_cms),miroc_baseline_model_calcs,color="green") +
  geom_line(aes(x=model_return_period_yr,y=model_flow_cms),hadley_baseline_model_calcs,color="blue") +
...
# csiro 4.5
csiro4_5_10yr_flow=flow_diff(10,csiro_baseline_model_calcs,csiro4_5_model_calcs)
csiro4_5_100yr_flow=flow_diff(100,csiro_baseline_model_calcs,csiro4_5_model_calcs)

# csiro 8.5
csiro8_5_10yr_flow=flow_diff(10,csiro_baseline_model_calcs,csiro8_5_model_calcs)
csiro8_5_100yr_flow=flow_diff(100,csiro_baseline_model_calcs,csiro8_5_model_calcs)

# hadley 4.5
hadley4_5_10yr_flow=flow_diff(10,hadley_baseline_model_calcs,hadley4_5_model_calcs)
hadley4_5_100yr_flow=flow_diff(100,hadley_baseline_model_calcs,hadley4_5_model_calcs)

# miroc 8.5
miroc8_5_10yr_flow=flow_diff(10,miroc_baseline_model_calcs,miroc8_5_model_calcs)
miroc8_5_100yr_flow=flow_diff(100,miroc_baseline_model_calcs,miroc8_5_model_calcs)
...
p1=ggplot(yadkin_subs_shp) +
  ...
  scale_fill_gradient2(name="% Change 10yr Flow",limits=c(-10,200),na.value="grey75")
...
# baseline
csiro_baseline_model_flood_freq_sel=csiro_baseline_model_calcs %>% 
  filter(RCH==8 | RCH==10 | RCH==18 | RCH==28) %>%
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="csiro_backcast")

miroc_baseline_model_flood_freq_sel=miroc_baseline_model_calcs %>% 
  filter(RCH==8 | RCH==10 | RCH==18 | RCH==28) %>%
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="miroc_backcast")

hadley_baseline_model_flood_freq_sel=hadley_baseline_model_calcs %>% 
  filter(RCH==8 | RCH==10 | RCH==18 | RCH==28) %>%
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="hadley_backcast")
...
csiro4_5_model_flood_freq_sel=csiro4_5_model_calcs %>% 
  ...
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="csiro4_5_projection")
...
csiro8_5_model_flood_freq_sel=csiro8_5_model_calcs %>% 
  ...
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="csiro8_5_projection")
...
hadley4_5_model_flood_freq_sel=hadley4_5_model_calcs %>% 
  ...
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="hadley4_5_projection")
...
miroc8_5_model_flood_freq_sel=miroc8_5_model_calcs %>% 
  ...
  filter(round(model_return_period_yr,4)==10 | round(model_return_period_yr,4)==100) %>%
  select(RCH:model_flow_log_cms) %>%
  mutate(dataset="miroc8_5_projection")

# bind together
all_flood_freq_models_sel=bind_rows(csiro_baseline_model_flood_freq_sel,
                                    miroc_baseline_model_flood_freq_sel,
                                    hadley_baseline_model_flood_freq_sel,
...
write_csv(all_flood_freq_models_sel,"model_flood_frequency_results_for_kelly_with_backcast.csv")
